...
parser.add_argument('-h', default='flexible', type=str, required=False, help='flexible, modelled_baseline, modelled_newbuild')
args = parser.parse_args()

percapita, node, iterations, population, nuclear_scenario, hydro_scenario = (args.e, args.n, args.i, args.p, args.s, args.h)
...
#factor_trans = np.genfromtxt('Data/factor_trans.csv', delimiter=',', usecols=1)

if nuclear_scenario == 'nuclear':
...
if hydro_scenario == 'flexible':
    ...
    hydro_weekly_cf = np.ones(MLoad.shape, dtype=np.float64)
hydromax_weeks = CHydro * hydro_weekly_cf[::7*24, :] *7*24/1000 # Weekly energy that can be generated by the hydro, GWh/week
...
def F(S):
    ...
    """ cost = (factor * np.array([S.CPV.sum(), S.CWind.sum(), GImports, S.CPHP.sum(), S.CPHS.sum()] + 
