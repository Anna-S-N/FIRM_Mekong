...
percapita, node, iterations, population, nuclear_scenario = (args.e, args.n, args.i, args.p, args.s)
...
phes_lb_np = np.array([0.] + [2*0.] + [0.] + [0.] + 5*[0.] + [1.] + [0.] + 7*[0.] + [0.]) # Lamtakong Jolabha Vadhana in Thailand (NEC) is 1000 MW
phes_ub_np = np.array([100000.] + 2*[100000.] + [100000.] + [100000.] + 7*[10000.] + 7*[0.] + [0.])
storage_lb_np = np.array(19*[0.])
storage_ub_np = np.array(12*[50000.] + 7*[0.])
...
hydromax_weeks = CHydro * hydro_weekly_cf[::7, :] *7*24/1000 # Weekly energy that can be generated by the hydro, GWh/week
...
DCloss = np.array([500, 200, 500, 500, 500, 200, 500, 500, 500, 200, 500,
                   500, 200, 500, 500, 500, 200, 500, 500, 500, 200, 500, 500]) * 0.07 * pow(10, -3) # UPDATE THIS TO CORRECT DISTANCES in km
...
if 'Grid' not in node:
    if 'TH' == node: # For MRC report, treat Thailand as a copper plate node like other regions
        ...
        hydromax_weeks = hydromax_weeks[np.where(Nodel==node)[0]] # GW
        #Hydromax = (EHydro.sum() + EBio.sum() + EWaste.sum())* pow(10,3) # GWh to MWh per year Not need in this version
...
if 'Grid' in node: 
    ...
    if 'TH_Iso' in node:
        ...
        #Full network of all node connections
        """ network = np.array([[11, 6], #SAC-CACW 
                            [6, 8], #CACW-MAC
                            [8, 5], #MAC-CACE
                            [8, 7], #MAC-CACN
                            [7, 9], #CACN-NAC
        ...
        if i == new_num_rows - 1:
            for j in range(2):
                for k in range(num_cols):
                    new_array[i, k] += original_array[i * 7 + j, k]
        else:
            for j in range(7):
                for k in range(num_cols):
                    new_array[i, k] += original_array[i * 7 + j, k]
...
def F(S):
    ...
    if 'Grid' in node:
        Deficit2 = Reliability(S, flexible=np.ones((intervals, nodes), dtype=np.float64) * CPeak * pow(10,3)) # Sj-EDE(t, j), MW 
        imports = np.clip(Deficit2,0,CInter.sum())
    else:
        imports = np.zeros(intervals)/efficiency
    ...
    # Deficit calculation
    Deficit = Reliability(S, flexible=hydrobio+imports) # Sj-EDE(t, j), GW to MW
