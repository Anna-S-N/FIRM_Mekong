...
def F(x):
    ...
    Hydro = min(0.5 * EHydro.sum() * pow(10, 3), Flexible) # GWh to MWh, MWh p.a. (other branch resolution * GHydro.sum() / years #)
    ...
    cost = factor * np.array([sum(S.CPV), sum(S.CWind), 0, sum(S.CPHP), S.CPHS] + list(CDC) + [sum(S.CPV), sum(S.CWind), Hydro * pow(10, -6), Fossil * pow(10, -6), -1, -1]) # $b p.a.
