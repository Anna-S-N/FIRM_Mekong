...
def F(x):
    ...
    # The following is a list representing the maximum electricity generation for each month (to be updated)
    max_generation_per_month = [100, 150, 120, 80, 200, 250, 180, 160, 130, 110, 90, 120]

    # Maximum annual electricity generated by existing capacity
    GHydro = resolution * hydro.sum() / years / efficiencyPH

    # Calculate monthly electricity generation based on the maximum for each month
    monthly_hydro_generation = GHydro / 12

    # Debug and adjust monthly generation based on the maximum allowable for each month
    for i in range(12):
        monthly_gas_generation = min(monthly_gas_generation, max_generation_per_month[i])
        monthly_hydro_generation = min(monthly_hydro_generation, max_generation_per_month[i])
